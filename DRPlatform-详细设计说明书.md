# DRPlatform 地理数据库异地灾备同步服务 — 详细设计说明书
...
| 项 | 内容 |
|---|------|
| 项目名称 | DRPlatform (remote-data-sync) |
| 文档名称 | 详细设计说明书 |
| 版本 | v3.0 |
| 状态 | 终稿（基于源码反向梳理 + 全模块覆盖） |
| 对应源码 | `remote-data-sync/src/main/java/com/example/RemoteDataSync/**` |
...
本文档是 **DRPlatform** 项目的全套技术设计说明书，目标读者为后续接手该项目的开发人员、运维人员和测试人员。文档力求做到"仅凭本文档 + 源码即可独立维护系统"，涵盖：
...
本文档覆盖 `remote-data-sync/remote-data-sync/` 模块的全部代码（Java 后端 + React 前端 + MySQL 元数据库 + Linux 部署脚本），不涉及 Flink 集群部署（预留 `DRPlatform.flink.enabled=true` 开关但当前版本未启用，使用自研内嵌同步引擎）。
...
DRPlatform 是一个 **MySQL 跨主机数据库灾备同步中间件**，核心能力：

1. **主机对模型**：配置源主机→目标主机（1:1），自动同步该源主机下所有用户库与表
2. **自动化库发现**：定时探测源主机所有用户库，排除系统库（information_schema/mysql/performance_schema/sys/DRPlatform）与忽略库
...
```
com.example.remotedatasync
├── RemoteDataSyncApplication              ← 启动类，@EnableScheduling
├── bean/                                ← 核心数据模型
...
│   ├── DRPlatformErrorCodeEnum.java      ← 10位错误码体系
...
│   └── DRPlatformProperties.java         ← DRPlatform.* 配置绑定
└── util/
...
元数据库 `DRPlatform` 存储同步进度、用户、操作日志等管控数据。与业务库（源/目标）完全隔离。
...
doSync()
  ...
  │   └── 排除系统库: information_schema, mysql, performance_schema, sys, DRPlatform
  └── for (String db : dbs)
...
└── properties: DRPlatformProperties          // 配置(maxPoolSize, maxGlobalConnections)
```
...
getDataSource(cfg)
  └── poolMap.computeIfAbsent(poolKey, k -> {
      ...
      hc.setMinimumIdle(2)
      hc.setPoolName("DRPlatform-" + poolKey)
...
├── DYNAMIC_MAPPINGS_FILE = "DRPlatform-dynamic-mappings.json"  // 持久化文件
└── properties: DRPlatformProperties                      // 配置（mappings 列表）
```
...
@PostConstruct startSyncTask()
  ...
  │   └── 读取 DRPlatform-dynamic-mappings.json，反序列化为 List<DatabaseMapping>
  │       for (m : loaded)
...
persistDynamicMappings()
  ...
  objectMapper.writerWithDefaultPrettyPrinter()
      .writeValue(new File("DRPlatform-dynamic-mappings.json"), toSave)

loadDynamicMappings()  // 启动时恢复
  f = new File("DRPlatform-dynamic-mappings.json")
  if (!f.exists()) return
...
```
SYSTEM_SCHEMAS = {information_schema, mysql, performance_schema, sys, DRPlatform}
...
步骤 5/8: 创建数据库并初始化表结构
  ├── CREATE DATABASE IF NOT EXISTS DRPlatform
...
步骤 6/8: 部署应用程序
  ├── 创建目录结构: /data/DRPlatform/{bin,logs,conf,conf/db,frontend,savepoint}
  ├── 复制 jar → /data/DRPlatform/bin/remote-data-sync.jar
  ├── 复制 init.sql → /data/DRPlatform/conf/db/
  ├── 复制前端资源 → /data/DRPlatform/frontend/
  └── 生成 application-linux.yml (含交互式配置的 MySQL/端口/同步源/目标)

步骤 7/8: 配置 systemd 服务
  ├── 停止旧服务 (如存在)
  ├── 生成 /etc/systemd/system/DRPlatform.service
  │   ├── ExecStart: java -Xms256m -Xmx512m -jar ... --spring.profiles.active=linux
  │   ├── Restart=on-failure, RestartSec=10
  │   └── StandardOutput/Error → /data/DRPlatform/logs/app.out/err
...
步骤 8/8: 配置 Nginx 并启动服务
  ...
  ├── Nginx 配置:
  │   ├── location / → 前端静态资源 (/data/DRPlatform/frontend)
...
============================================================
  ...
  常用命令:
    启动:  systemctl start DRPlatform
    停止:  systemctl stop DRPlatform
    重启:  systemctl restart DRPlatform
    状态:  systemctl status DRPlatform
    日志:  journalctl -u DRPlatform -f
           tail -f /data/DRPlatform/logs/app.log

  卸载:
    systemctl stop DRPlatform && systemctl disable DRPlatform
    rm -f /etc/systemd/system/DRPlatform.service /etc/nginx/conf.d/DRPlatform.conf
...
ExecStart=/usr/bin/java -Xms256m -Xmx512m -jar /data/DRPlatform/bin/remote-data-sync.jar \
  --spring.profiles.active=linux \
  --spring.config.additional-location=/data/DRPlatform/conf/
...
bash /d/WorkSpace/remote-data-sync/mvn3.sh -o clean package -DskipTests

# 启动（必须显式指定端口，因为环境变量 SERVER__PORT=9591 会覆盖）
unset SERVER__PORT SERVER_PORT
/d/software/jdk-21.0.11/bin/java -jar target/remote-data-sync.jar --server.port=8080
...
| 动态映射丢失 | `DRPlatform-dynamic-mappings.json` 文件保存，重启后 `loadDynamicMappings()` 加载 |
...
DRPlatform 项目采用分层自动化测试策略，共计 **188 个自动化测试用例**，覆盖 API 接口、同步引擎、配置热生效、用户隔离与审计四大维度。
...
参考 `DRPlatformConsistencyTest.java`（源码已实现的核心集成测试）：
...
# 单独执行集成测试
bash /d/WorkSpace/remote-data-sync/mvn3.sh -o test \
  -Dtest=DRPlatformConsistencyTest \
...
| 参数 | 默认值 | 说明 |
|------|--------|------|
| `DRPlatform.engine.poll-interval-ms` | 2000 | 同步轮询间隔 |
| `DRPlatform.engine.batch-size` | 1000 | 批次大小 |
| `DRPlatform.engine.max-queue-size` | 20000 | 表级队列上限 |
| `DRPlatform.connection.max-pool-size` | 32 | 每分片连接池上限 |
| `DRPlatform.connection.max-global-connections` | 200 | 全局连接数上限 |
| `DRPlatform.standby-mode` | true | 灾备中心模式开关 |
| `DRPlatform.flink.enabled` | false | Flink 集群模式开关（当前未启用） |
...
| 集成测试 | `test/.../DRPlatformConsistencyTest.java` | 360 |
...
A: 项目预留了 `DRPlatform.flink.enabled` 开关，可切换到 Flink 集群模式。当前版本使用自研内嵌引擎的原因：

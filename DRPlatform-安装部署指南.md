...
DRPlatform 是 MySQL 跨主机数据库灾备同步中间件，采用 Spring Boot 单体架构，部署时需要 Java 17+ 运行环境和 MySQL 5.7+ 数据库。系统支持 MySQL 5.7 与 8.0 之间的任意方向同步（8→8、5→5、8→5、5→8），跨版本场景下自动处理 DDL 语法兼容。本文档覆盖从环境准备到服务上线的完整流程，包含一键安装、手动安装、升级、卸载四个场景。

## 环境要求

### 操作系统

DRPlatform 基于 Java 运行，理论上支持任何能运行 JDK 17 的操作系统。已在以下环境验证：
...
DRPlatform 支持 MySQL 5.7 和 8.0 之间的任意方向同步。四种版本组合均已验证：
...
源端 MySQL 必须开启 binlog 并设置 `binlog_format=ROW`，这是灾备同步的基础前提。DRPlatform 的 `SourceBinlogJob` 定时任务（每 30 秒）通过 `SHOW MASTER STATUS` 采集源端 binlog 位点（File + Position），用于：
...
如果 `SHOW MASTER STATUS` 返回空，说明 binlog 未开启，需修改 `my.cnf` 后重启 MySQL。`binlog_format` 不是 `ROW` 时（如 `STATEMENT` 或 `MIXED`），虽然 DRPlatform 的数据同步不直接解析 binlog 内容，但会影响灾备恢复时的数据精度和 HA 切换检测的准确性。
...
如果服务器上已有其他应用占用 80 或 443 端口（如宝塔面板、其他 Web 应用），DRPlatform 必须使用独立端口。8088 是推荐值，可通过 install.sh 交互配置或手动修改 Nginx 配置更改。
...
```sql
-- 元数据库 DRPlatform 的完整权限
GRANT ALL PRIVILEGES ON DRPlatform.* TO 'root'@'%';
...
生产环境建议为 DRPlatform 创建专用账号而非使用 root，但需确保该账号对源端所有用户库有 SELECT 权限、对目标端所有用户库有 CREATE/INSERT/UPDATE/DELETE/ALTER/DROP 权限。
...
| 文件 | 来源 | 说明 |
|------|------|------|
| `remote-data-sync.jar` | `mvn -DskipTests package` 构建产物 | 应用主包，约 80 MB |
...
```bash
cd D:\WorkSpace\remote-data-sync\remote-data-sync
mvn -DskipTests package -q
```

构建产物位于 `target/remote-data-sync.jar`。将此文件与 `deploy/install.sh`、`src/main/resources/db/init.sql` 一起上传到目标服务器。
...
```bash
# 从本地上传到服务器
scp remote-data-sync.jar root@192.168.88.88:/root/deploy/
...
========== 步骤 1/8: 前置检查 ==========
[INFO] root 权限确认 OK
[INFO] 找到 JAR 包: /root/deploy/remote-data-sync.jar
...
**步骤 5/8 — 创建数据库并初始化表结构**：创建 `DRPlatform` 元数据库，执行 `init.sql` 创建 6 张管控表（sync_progress、dbha_switch_status、source_latest_binlog_info、sync_restart_task、sys_user、operation_log），并为 sync_progress 添加 user_id 列。

**步骤 6/8 — 部署应用程序**：创建 `/data/DRPlatform/` 目录结构，复制 jar 包和前端资源，根据输入的凭据生成 `application-linux.yml` 配置文件。

**步骤 7/8 — 配置 systemd 服务**：创建 `/etc/systemd/system/DRPlatform.service`，配置开机自启和故障自动重启。
...
============================================================
  ...
  配置文件:    /data/DRPlatform/conf/application-linux.yml
  日志目录:    /data/DRPlatform/logs/
  数据目录:    /data/DRPlatform/
============================================================
```
...
# 检查配置文件是否在正确位置
ls -la /www/server/panel/vhost/nginx/DRPlatform.conf
ls -la /www/server/nginx/conf/vhost/DRPlatform.conf
```
...
```bash
mv /www/server/nginx/conf/vhost/DRPlatform.conf \
   /www/server/panel/vhost/nginx/DRPlatform.conf
nginx -t && systemctl restart nginx
```

### 端口冲突处理

宝塔面板环境下 80 和 443 端口通常已被其他应用占用。如果 `DRPlatform.conf` 中 `listen 80;` 与其他 vhost 的 `server_name _;` 冲突，Nginx 会发出警告：

```
[warn] conflicting server name "_" on 0.0.0.0:80, ignored
```

此时 DRPlatform 的配置会被忽略，80 端口的请求被其他应用拦截。解决方案是让 DRPlatform 使用独立端口：

```nginx
# 编辑 /www/server/panel/vhost/nginx/DRPlatform.conf
server {
    listen 8088;          # 改为独立端口
    server_name _;
    # ... 其余配置不变
...
init.sql 会创建 `DRPlatform` 元数据库及 6 张管控表，同时创建示例业务库 `geo_source`、`geo_target`、`biz_source`、`biz_target`。
...
```bash
# 创建目录结构
mkdir -p /data/DRPlatform/{bin,logs,conf,conf/db,frontend,savepoint}

# 复制文件
cp remote-data-sync.jar /data/DRPlatform/bin/
cp init.sql /data/DRPlatform/conf/db/

# 上传前端资源（index.html 和 app.js）
# 从构建机复制 frontend/ 目录下的 index.html 和 app.js
cp index.html app.js /data/DRPlatform/frontend/
```

### 5. 创建配置文件

创建 `/data/DRPlatform/conf/application-linux.yml`，将占位符替换为实际值：
    ...
    resources:
      cache:
        cachecontrol:
          ...
          max-age: 0
  datasource:
    url: jdbc:mysql://<MYSQL_HOST>:<MYSQL_PORT>/DRPlatform?useUnicode=true&characterEncoding=utf8&serverTimezone=Asia/Shanghai&allowPublicKeyRetrieval=true&useSSL=false
    username: <MYSQL_USER>
    password: <MYSQL_PASS>
...
DRPlatform:
  ...
  flink:
    ...
    savepoint-path: /data/DRPlatform/savepoint
  engine:
...
logging:
  ...
  file:
    name: /data/DRPlatform/logs/app.log
```

### 6. 配置 systemd 服务

创建 `/etc/systemd/system/DRPlatform.service`：
...
ExecStart=/usr/bin/java -Xms256m -Xmx512m -jar /data/DRPlatform/bin/remote-data-sync.jar --spring.profiles.active=linux --spring.config.additional-location=/data/DRPlatform/conf/
SuccessExitStatus=143
Restart=on-failure
RestartSec=10
StandardOutput=append:/data/DRPlatform/logs/app.out
StandardError=append:/data/DRPlatform/logs/app.err
...
| 文件 | 用途 |
|------|------|
| `/etc/systemd/system/DRPlatform.service` | systemd 服务定义 |
| `/www/server/panel/vhost/nginx/DRPlatform.conf` | Nginx vhost（宝塔面板） |
| `/etc/nginx/conf.d/DRPlatform.conf` | Nginx vhost（标准安装） |
...
主配置文件 `/data/DRPlatform/conf/application-linux.yml` 在安装时由 install.sh 根据用户输入生成。核心配置项：
...
# 2. 备份旧 jar
cp /data/DRPlatform/bin/remote-data-sync.jar /data/DRPlatform/bin/remote-data-sync.jar.bak

# 3. 替换 jar 包
cp /root/deploy/remote-data-sync.jar /data/DRPlatform/bin/

# 4. 如有新版前端资源，一并更新
cp app.js index.html /data/DRPlatform/frontend/
...
```bash
# 备份
mysqldump -u root -p DRPlatform > /tmp/DRPlatform_backup_$(date +%Y%m%d).sql

# 执行升级 SQL（参考新版本的 release notes）
mysql -u root -p DRPlatform < upgrade_v2.1.sql
```
...
| `Access denied for user` | MySQL 权限不足 | 检查账号是否有 DRPlatform.* 的 ALL 权限 |
...
# 宝塔面板: /www/server/panel/vhost/nginx/DRPlatform.conf
# 标准安装: /etc/nginx/conf.d/DRPlatform.conf
...
```bash
# 检查前端文件是否存在
ls -la /data/DRPlatform/frontend/
...
# 查看应用日志中的同步引擎日志
grep -E "SyncHostJob|ScanNewDatabaseJob" /data/DRPlatform/logs/app.log | tail -50
...
```bash
# 查看应用日志中的 DDL 相关错误
grep -E "DDL|sanitize|ALTER|CREATE TABLE" /data/DRPlatform/logs/app.log | grep -i error | tail -20
...
# 3. 删除 Nginx 配置
rm -f /www/server/panel/vhost/nginx/DRPlatform.conf   # 宝塔面板
# 或
rm -f /etc/nginx/conf.d/DRPlatform.conf                # 标准安装
systemctl restart nginx
...
mysql -u root -p -e "DROP DATABASE DRPlatform;"
```
...
| 检查项 | 命令 | 预期结果 |
|--------|------|----------|
| systemd 服务 | `systemctl is-active DRPlatform` | `active` |
...
| 数据库表 | `SHOW TABLES FROM DRPlatform;` | 6 张管控表 |

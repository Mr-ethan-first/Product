#!/bin/bash
# =============================================================================
# GeoDRSync 灾备同步服务 - CentOS7 一键部署脚本
...
SRC_JAR="../flink-cdc-sync/target/${JAR_NAME}"
...
# 前端静态资源
[ -d "../flink-cdc-sync/frontend" ] && cp -rf ../flink-cdc-sync/frontend/* "${APP_DIR}/frontend/"
# 初始化 SQL
[ -d "${APP_DIR}/conf/db" ] || mkdir -p "${APP_DIR}/conf/db"
cp -f ../flink-cdc-sync/src/main/resources/db/init.sql "${APP_DIR}/conf/db/init.sql"

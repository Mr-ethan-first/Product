...
# 复制前端静态资源
for fe_dir in "${SCRIPT_DIR}/../frontend" "${SCRIPT_DIR}/frontend" "${SCRIPT_DIR}/../flink-cdc-sync/frontend"; do
    if [ -d "${fe_dir}" ]; then
...
# 启动后端服务
info "启动 GeoDRSync 服务..."
systemctl restart "${APP_NAME}"
sleep 5

# 验证服务状态
if systemctl is-active --quiet "${APP_NAME}"; then
    info "GeoDRSync 服务已启动 (running)"
else
    error "GeoDRSync 服务启动失败"
...
echo ""
echo "============================================================"
echo -e "${GREEN}  GeoDRSync 灾备同步服务部署完成！${NC}"
echo "============================================================"
echo ""
echo "  访问地址:"

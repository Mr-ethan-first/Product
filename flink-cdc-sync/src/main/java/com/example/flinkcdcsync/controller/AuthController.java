...
public class AuthController {
    ...
    public Result<Void> register(@RequestBody Map<String, String> body, HttpServletRequest request) {
        ...
        if (!StringUtils.hasText(username) || !StringUtils.hasText(password)) {
            throw new BusinessException(GeoDRSyncErrorCodeEnum.AUTH_PARAM.getCode(), "用户名与密码均不能为空");
        }
        if (username.length() < 3 || username.length() > 32) {
            throw new BusinessException(GeoDRSyncErrorCodeEnum.AUTH_PARAM.getCode(), "用户名长度需在 3-32 之间");
        }
        if (password.length() < 6) {
            throw new BusinessException(GeoDRSyncErrorCodeEnum.AUTH_PARAM.getCode(), "密码长度至少 6 位");
        }
        if (sysUserMapper.selectCount(new LambdaQueryWrapper<SysUser>().eq(SysUser::getUsername, username)) > 0) {
            throw new BusinessException(GeoDRSyncErrorCodeEnum.AUTH_DUPLICATE.getCode(), "该用户名已被注册");
        }
    ...
    public Result<Map<String, Object>> login(@RequestBody Map<String, String> body, HttpServletRequest request) {
        ...
        if (u == null || !PasswordUtil.matches(password, u.getPasswordHash())) {
            throw new BusinessException(GeoDRSyncErrorCodeEnum.AUTH_FAIL.getCode(), "用户名或密码错误");
        }
     ...
     * 真正的业务接口（/sync/**）未登录时仍由 {@link com.example.flinkcdcsync.manager.AuthInterceptor} 返回 401。</p>
     */
    @GetMapping("/me")

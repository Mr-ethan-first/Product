...
import com.example.flinkcdcsync.sync.SyncHostJob;
...
public class DatabaseSyncManager {
    ...
    private static final String DYNAMIC_MAPPINGS_FILE = "geodrsync-dynamic-mappings.json";
    ...
    public DatabaseSyncManager(GeoDRSyncProperties properties,
    ...
    public MappingBatchResult addMappings(List<DatabaseMapping> mappings) {
        ...
        // 绑定当前操作用户 ID
        Long currentUserId = com.example.flinkcdcsync.common.UserContext.getUserId();
        synchronized (mappingsLock) {
            for (DatabaseMapping mapping : mappings) {
                String key = mapping.getInstanceKey();
                if (findByKey(key) != null) {
        ...
        synchronized (mappingsLock) {
            ...
            if (configUpdate.containsKey("transformRules")) {
                ...
                if (val instanceof List) {
                    List<com.example.flinkcdcsync.bean.TransformRule> rules = new ArrayList<>();
                    for (Object item : (List<?>) val) {
                        if (item instanceof Map) {
                            Map<String, Object> m = (Map<String, Object>) item;
                            com.example.flinkcdcsync.bean.TransformRule rule = new com.example.flinkcdcsync.bean.TransformRule();
            ...
            if (configUpdate.containsKey("ignoreTablesByDb")) {
                ...
                if (val instanceof List) {
                    List<com.example.flinkcdcsync.bean.DbTableIgnore> ignores = new ArrayList<>();
                    for (Object item : (List<?>) val) {
                        if (item instanceof Map) {
                            Map<String, Object> m = (Map<String, Object>) item;
                            com.example.flinkcdcsync.bean.DbTableIgnore ignore = new com.example.flinkcdcsync.bean.DbTableIgnore();
